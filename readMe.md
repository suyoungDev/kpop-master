...
- for in으로 리덕스의 state값 user Array에서 isAuth가 있는가 판단하는건?
  - 그럼 error값이 return되어도 boolean일 것.
  - 일단 해보고 결정
- 그냥 auth (node쪽) 코드 작성시 토큰이 없으면 error도 함께 반환하는 게 아니라 isAuth:false 만 반환하는 건?
- 그것도 안되면 authContext처럼 기본값이 false가 아니라 undefined인건?
